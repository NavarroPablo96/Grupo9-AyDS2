...
public class ComunicadorRegistro implements IComunicador{
    ...
    void enviarEvento(Evento evento) {
    	System.out.println("Comunicador-39 - Se envia el evento: "+evento.getClass().getName());
        if (!estaConectado()) {
	...
	private void conectarSecundario() throws UnknownHostException, IOException {
		...
		new Thread(() -> {
			try {
		        ...
		        e.printStackTrace();
				System.out.println("Hubo exception en ComunicacionEntreProcesos157");
			} 
		}).start();
	}
	private void conectarPrimario() throws UnknownHostException, IOException {
		...
		new Thread(() -> {
			try {
		        ...
		        e.printStackTrace();
				System.out.println("Hubo exception en ComunicacionEntreProcesos-184");
			}
		}).start();
	}
